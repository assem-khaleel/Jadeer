...
class Orm_Sp_Goal extends Orm
{

    /**
     * @var $instances Orm_Sp_Goal[]
     */
    protected static $instances = array();
    protected static $table_name = 'sp_goal';
    ...
    /**
     * @return Sp_Goal_Model
     */
    public static function get_model()
    {
        return Orm::get_ci_model('Sp_Goal_Model');
    }
    ...
    public function save() {
        ...
        if($this->check_object_field('lead') || $this->check_object_field('lag')) {
            $this->get_strategy_obj()->compute_progress();
        }
    ...
    /**
     * this function prepare children
     * @return $this the call function
     */
    public function prepare_children()
    {
        if(!self::$prepare_children) {

            $goals = self::get_all(array('goal_id' => $this->get_goal_id(), 'parent_lft' => $this->get_parent_lft(), 'parent_rtl' => $this->get_parent_rtl()));

            if ($goals) {
                foreach ($goals as $goal) {
                    if ($goal->get_parent_id()) {
                        $parent = self::get_instance($goal->get_parent_id());
    ...
    /**
     * this function add child by its goal
     * @param Orm_Sp_Goal $goal the goal of the add child to be call function
     * @return mixed the call function
     */
    public function add_child(Orm_Sp_Goal $goal) {
        $this->children[$goal->get_id()] = $goal;
    }
    /**
     * this function get child
     * @return Orm_Sp_Goal[] the call function
     */
    ...
    /**
     * this function build parent tree
     */
    public function build_parent_tree() {

        $old_id = $this->get_goal_id();

        if($this->get_parent_id()) {
            $this->set_goal_id($this->get_parent_obj()->get_goal_id());
        } else {
            $this->set_goal_id($this->get_id());
        }

        $this->save();

        self::get_model()->update_group_tree($old_id ,$this->get_goal_id(), $this->get_parent_lft(), $this->get_parent_rtl());
        self::get_model()->build_parent_tree($this->get_goal_id());
    }
    /**
     * this function get parent obj
     * @return Orm_Sp_Goal the object call function
     */
    ...
    /**
     * this function get strategy obj
     * @return orm_Sp_Strategy the object call function
     */
    public function get_strategy_obj()
    {
        return Orm_Sp_Strategy::get_instance($this->strategy_id);
    }
    ...
    public function get_progress_lag($last_period = false) {
        ...
        if(!isset($this->progress_lag[$period])) {
            $this->progress_lag[$period] = Orm_Sp_Strategy::get_progress_type('goal', $this->get_id(), 'lag', $last_period);
        }
    ...
    public function get_target_lag() {
        ...
        if(is_null($this->target_lag)) {
            $this->target_lag = Orm_Sp_Strategy::get_target_type('goal', $this->get_id());
        }
    ...
    public function get_progress_lead($last_period = false) {
        ...
        if(!isset($this->progress_lead[$period])) {
            $this->progress_lead[$period] = Orm_Sp_Strategy::get_progress_type('goal', $this->get_id(), 'lead', $last_period);
        }
    ...
    public function get_target_lead() {
        ...
        if(is_null($this->target_lead)) {
            $this->target_lead = Orm_Sp_Strategy::get_target_type('goal', $this->get_id());
        }
     ...
     * @return Orm_Sp_Objective[] the call function
     */
    public function get_objectives() {
        if(is_null($this->objectives)) {
            $this->objectives = Orm_Sp_Objective::get_all(array('goal_id' => $this->get_id()));
        }
