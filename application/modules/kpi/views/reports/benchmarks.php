...
<div class="table-primary">
    <div class="table-header">
        <span class="table-caption">
            <?php echo lang('KPIs Benchmarks Report'); ?>
        </span>
        <?php if (!isset($export)) { ?>
            <div class="panel-heading-controls col-sm-4"><a href="/kpi/report/<?php echo Orm_Kpi::KPI_LIST_REPORT_NORMAL ?>/1/<?php echo $is_strategic ?>" class=" btn btn-sm pull-right"><i class="btn-label-icon left fa fa-file-pdf-o"></i><?php echo lang('PDF'); ?></a></div>
        <?php } ?>
    </div>
    <?php if (!isset($export)) { ?>
        <div class="table-header">
            <form method="GET">
                <div class="row">
                    <div class="col-md-3" style="margin-bottom: 10px;">
                        <a class="btn btn-md btn-block" href="/kpi/report/<?php echo Orm_Kpi::KPI_LIST_REPORT_NORMAL ?>" type="reset" ><?php echo lang('Institution') ?></a>
                    </div>
                    <div class="col-md-3" style="margin-bottom: 10px;">
    ...
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <th class="col-md-4"><?php echo lang('KPI Title'); ?></th>
            <th class="col-md-1"><?php echo lang('Unit Responsible'); ?></th>
            <th class="col-md-1"><?php echo lang('Stakeholder'); ?></th>
            <th class="col-md-1"><?php echo lang('Actual Benchmark'); ?></th>
            <th class="col-md-1"><?php echo lang('Target Benchmark'); ?></th>
            <th class="col-md-1"><?php echo lang('Institution Benchmark'); ?></th>
            <th class="col-md-1"><?php echo lang('College Benchmark'); ?></th>
            <th class="col-md-1"><?php echo lang('New Benchmark'); ?></th>
            <th class="col-md-1"><?php echo lang('External Benchmarks'); ?></th>
        </tr>
        </thead>
        <tbody>
        <?php if (!$is_strategic) { ?>
            <?php foreach (Orm_Standard::get_all() as $standard) { ?>
                ...
                <?php $current_kpi = 0; ?>
                <?php foreach (Orm_Kpi::get_all(array('standard_id' => $standard->get_id(),'only_college_id' => 0)) as $kpi) { ?>
                    <?php
                    $exist = false;
                    ...
                    if (count($levels)) { ?>
                        <?php foreach ($levels as $level) { ?>
                            <tr>
                                ...
                                <?php $values = $level->get_rows($type,$fltr,Orm_Kpi::get_instance(0)); ?>
                                <?php foreach ($values as $value) { ?>
                                    <td class="text-xs">
                                        <?php foreach ($value as $key => $item) { ?>
                                            <?php if (is_array($item)) { ?>
                                                <?php echo $key . ': ('; ?>
                                                <?php foreach ($item as $external_key => $external_value) { ?>
                                                    <?php echo ($external_key ? ',' : '') . $external_value; ?>
                                                <?php } ?>
                                                <?php echo ')'; ?>
                                            <?php } else { echo $key .': ' . $item; } ?>
                                            <br>
                                        <?php } ?>
                                    </td>
                                <?php } ?>
