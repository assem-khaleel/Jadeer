<?php
/** @var $domains Orm_Cm_Learning_Domain[] */
...
<div class="table-primary">
    ...
    <div class="table-header">
        <div class="table-caption"><?php echo htmlfilter(Orm_Course::get_instance($course_id)->get_code() . ' - ' . Orm_Course::get_instance($course_id)->get_name()); echo htmlfilter($section_id ? ' ( '.lang('Section #').': '.Orm_Course_Section::get_instance($section_id)->get_section_no().' ) ' : ''); ?></div>
    </div>
    <table class="table table-bordered  table-responsive">
            ...
            <tr class="text-bold">
                <td colspan="<?php echo count($course_assessment_methods) + 2; ?>">
                    <div class="alert alert-default">
                        <div class="m-b-1">
                            <?php echo lang('This Course has not been assessed yet'); ?>
                        </div>
                    </div>
                </td>
            </tr>
        <?php } else { ?>
            <?php foreach ($domains as $domain) { ?>
                <tr class="text-bold">
                    <td>
                        <b><a href="javascript: void(0);" onclick="load_clo(<?php echo $domain->get_id(); ?>)"><?php echo htmlfilter($domain->get_title()); ?></a> </b>
                    </td>
                    <?php foreach ($course_assessment_methods as $method) { ?>
                        <td class="text-right"><?php echo Orm_Cm_Section_Student_Assessment::get_course_assessment_method_score($course_id,$method->get_id(),$domain->get_id(), 0,$section_id); ?></td>
                    <?php } ?>
                    <td class="text-right"><?php $domains_array[$domain->get_id()] = array($domain->get_title(), Orm_Cm_Section_Student_Assessment::get_course_assessment_method_score($course_id,0,$domain->get_id(), 0,$section_id)); echo $domains_array[$domain->get_id()][1] ?></td>
                </tr>
                <?php foreach (Orm_Cm_Course_Learning_Outcome::get_outcomes($course_id, $domain->get_id()) as $course_learning_outcome) { ?>
                    <tr>
                        ...
                        <?php foreach ($course_assessment_methods as $method) { ?>
                            <td class="text-right"><?php echo Orm_Cm_Section_Student_Assessment::get_course_assessment_method_score($course_id, $method->get_id(), $domain->get_id(), $course_learning_outcome->get_id(),$section_id); ?></td>
                        <?php } ?>
                        <td class="text-right"><?php echo Orm_Cm_Section_Student_Assessment::get_course_assessment_method_score($course_id, 0, $domain->get_id(), $course_learning_outcome->get_id(),$section_id); ?></td>
                    </tr>
                <?php } ?>
            <?php } ?>
            <tr class="text-bold bg-default">
                ...
                <?php foreach ($course_assessment_methods as $method) { ?>
                    <td class="text-right"><?php echo Orm_Cm_Section_Student_Assessment::get_course_assessment_method_score($course_id, $method->get_id(), 0, 0,$section_id); ?></td>
                <?php } ?>
                <td class="text-right"><?php echo Orm_Cm_Section_Student_Assessment::get_course_assessment_method_score($course_id, 0, 0, 0,$section_id); ?></td>
            </tr>
        <?php } ?>
        ...
        <div class="table-footer">
            ...
            <script type="text/javascript">
                ...
                function load_clo(domain_id) {
                    $('#clo-chart').load('/curriculum_mapping/reporting/clo_results/<?php echo $course_id ?>/<?php echo $section_id ?>/'+domain_id).css('display','block');
                }
            </script>
