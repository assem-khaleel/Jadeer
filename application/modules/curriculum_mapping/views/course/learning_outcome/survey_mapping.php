...
/** @var Orm_Cm_Course_Learning_Outcome $clo */
?>
<style>
    .jstree-anchor {
        /*enable wrapping*/
...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <?php echo form_open("/curriculum_mapping/course/clo_survey/{$clo->get_id()}", array('id'=>'clo-survey-form')) ?>
                                        ...
                                        <li id="f_<?php echo intval($factor->get_id()); ?>"><?php echo trim(htmlfilter($factor->get_title())); ?>
                                            <ul>
                                                <?php foreach (Orm_Survey_Question_Statement::get_all(array('factor_id' => $factor->get_id())) as $statement) { ?>
                                                    <li <?php echo(Orm_Cm_Course_Learning_Outcome_Survey::get_one(array('course_learning_outcome_id' => $clo->get_id(), 'survey_id' => $survey->get_id(), 'factor_id' => $factor->get_id(), 'statement_id' => $statement->get_id()))->get_id() ? 'data-checkstate="checked"' : ''); ?>
...
<script>
    ...
    $('form#clo-survey-form').submit(function (event) {
        event.preventDefault();
        $('#selected_items').val(JSON.stringify($('#clo-survey').jstree("get_selected")));
        $.ajax({
