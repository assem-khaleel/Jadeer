...
<div class="px-content">
        ...
        if (!$results) {
            ...
            <div class="alert alert-default alert-dark">
                <?php if (!empty($searchText)) { ?>
                    ...
                    <p><?php echo  lang('Note');?> : <?php echo  lang('You can search for a faculty member by his/her name or email');?></p>
                <?php } else { ?>
                    <h4 class="alert-heading"><?php echo  lang('Begin by searching for a faculty member.');?></h4>
                    <p><?php echo  lang('Note');?> : <?php echo  lang('You can search for a faculty member by his/her name or email');?></p>
                <?php } ?>
            </div>
        <?php } ?>
    ...
    <div class="tab-content p-y-0">
        <?php
        if ($count !=0){?>
            <div class="m-b-3 text-muted font-size-12"><strong><?php echo  $count; ?> </strong><?php echo  lang('Faculty are Founded');?></div>

         <?php foreach ($results as $result) {
                $user = Orm_User_Faculty::get_instance($result->get_id()); ?>
                <div class="tab-pane fade in active" id="results-pages">
                    <div class="panel">
                        <div class="panel-body">
                            <div class="font-weight-semibold font-size-16 p-b-1">
                                <a href="/faculty_portfolio/view/<?php echo  $result->get_id() ?>"><?php echo  htmlfilter($result->get_first_name()) . ' ' . htmlfilter($result->get_last_name()) ?></a>
                            </div>
