 ...
 */
/** @var string $pager */
/** @var Orm_Data_Course_Students[] $items */
/** @var array $fltr */
?>
<?php if (!isset($pdf) || !$pdf) { ?>
    <div class="box bg-primary p-a-2">
        ...
        <div class="pull-right">
            <a href="/statistics/course_students/1/<?php echo $query_string; ?>" class="btn btn-sm text-default"><span class="btn-label-icon left icon fa fa-file-pdf-o"></span> <?php echo lang('PDF') ?></a>
        </div>
    </div>
        ...
        <div id="filters" class="collapse in" aria-expanded="true" style="">
            <div class="well">
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <select name="fltr[semester_id]" class="form-control">
                    ...
                    <div class="col-sm-9">
                        <select id="course_block" name="fltr[course_id]" class="form-control" onchange="get_sections_by_course(this, $('#semester_id').val(), 1)">
                            ...
                            <?php if (isset($fltr['program_id'])) { ?>
                                <?php foreach (Orm_Program_Plan::get_all(array('program_id' => $fltr['program_id'])) as $plan) { ?>
                                    <?php $selected = $plan->get_course_id() == $fltr['course_id'] ? 'selected="selected"' : ''; ?>
                                    <option value="<?php echo $plan->get_course_id(); ?>" <?php echo $selected; ?>><?php echo htmlfilter($plan->get_course_obj()->get_code()) . ' - ' . htmlfilter($plan->get_course_obj()->get_name()); ?></option>
                                <?php } ?>
                            <?php } ?>
                        </select>
                    </div>
                        ...
                        <select id="section_block" name="fltr[section_id]" class="form-control">
                            ...
                            <?php if (isset($fltr['course_id']) && isset($fltr['semester_id'])) { ?>
                                <?php foreach (Orm_Course_Section::get_all(array('course_id' => $fltr['course_id'], 'semester_id' => $fltr['semester_id'])) as $section) { ?>
                                    ...
                                    <option value="<?php echo $section->get_id(); ?>" <?php echo $selected; ?>><?php echo htmlfilter($section->get_section_no()); ?></option>
                                <?php } ?>
                            <?php } ?>
                        </select>
                    </div>
...
<div class="table-primary">
    <div class="table-header">
        <div class="table-caption"><?php echo lang('Course Students'); ?></div>
    </div>
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <th><?php echo lang('Completing') ?></th>
        ...
        <?php foreach ($items as $item) { ?>
            <tr>
                ...
                <td><?php echo htmlfilter($item->get_section_id()); ?></td>
                <td><?php echo $item->get_student_start_count() ?></td>
                <td><?php echo $item->get_student_complete_count(); ?></td>
                <td><?php echo round($item->get_student_start_count() ? $item->get_student_complete_count() / $item->get_student_start_count() : 0,2); ?></td>
            </tr>
        <?php } ?>
        <?php if(empty($items)) { ?>
            <tr>
