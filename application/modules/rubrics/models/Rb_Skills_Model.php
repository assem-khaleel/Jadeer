...
class Rb_Skills_Model extends CI_Model {
    ...
    */
    public function get_all($filters = array(), $page = 0, $per_page = 10, $orders = array(), $fetch_as = Orm::FETCH_OBJECTS) {
        ...
        $this->db->select('rs.*');
        $this->db->distinct();
        $this->db->from(Orm_Rb_Skills::get_table_name() . ' AS rs');
        $this->db->where('rs.is_deleted', 0);
        ...
        if (!empty($filters['extra_data'])) {
            $this->db->where('rs.extra_data', $filters['extra_data']);
        }
        ...
        if (isset($filters['or_name_en'])||isset($filters['or_name_ar'])||isset($filters['or_value'])) {
            ...
            if (isset($filters['or_value'])) {
                $this->db->or_where('rs.value', $filters['or_value']);
            }
        ...
        switch($fetch_as) {
            ...
            return Orm_Rb_Skills::to_object($this->db->get()->row_array());
            break;
            case Orm::FETCH_OBJECTS:
            $objects = array();
            foreach($this->db->get()->result_array() as $row) {
                $objects[] = Orm_Rb_Skills::to_object($row);
            }
            return $objects;
            break;
    ...
    */
    public function insert($params = array()) {
        $this->db->insert(Orm_Rb_Skills::get_table_name(), $params);
    ...
    */
    public function update($id, $params = array()) {
        return $this->db->update(Orm_Rb_Skills::get_table_name(), $params, array('id' => $id));
    }
    ...
    */
    public function delete($id) {
        return $this->db->delete(Orm_Rb_Skills::get_table_name(), array('id' => $id));
//        return $this->db->update(Orm_Rb_Skills::get_table_name(), array('is_deleted' => 1), array('id' => $id));
    }
