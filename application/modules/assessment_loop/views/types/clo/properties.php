...
<div class="row form-group">
    <label for="type_id" class="col-sm-3 control-label"><?php echo lang('Learning Domain Types'); ?></label>
    <div class="col-sm-9">
...
<div class="row form-group">
    ...
    <div class="col-sm-9">
        <select id ="domain_block_wizard" name="extra_data[learning_domain]" class="form-control" onchange="select_leaning_domain()();">
            ...
            <?php
            foreach(Orm_Cm_Learning_Domain::get_all(array('type_id'=> $assessment_loop->get_type_id())) as $domain) {
...
<div class="row form-group">
    ...
    <div class="col-sm-9">
        <select name="extra_data[program_learning_outcome]" id="plo_selector" class="form-control" onchange="select_plos();">
            <option value="-1"><?php echo lang('Select One'); ?></option>
            <?php
            foreach(Orm_Cm_Program_Learning_Outcome::get_all(['program_id' => $assessment_loop->get_item_type_id(), 'learning_domain_id'=>$assessment_loop->get_item_from_extra_data('learning_domain')]) as $plo) {
                $selected = (!is_null($assessment_loop->get_item_from_extra_data('program_learning_outcome')) && $plo->get_id() == $assessment_loop->get_item_from_extra_data('program_learning_outcome')) ? 'selected="selected"' : '';
                ?>
...
<div class="row form-group">
    ...
    <div class="col-sm-9">
        <select name="extra_data[course]" id="course_selector" class="form-control" onchange="select_course();">
            ...
            $courses = Orm_Cm_Course_Learning_Outcome::get_model()->get_all([
                    'learning_domain_id'=>$assessment_loop->get_item_from_extra_data('learning_domain')?:-1,
                    'program_learning_outcome_id'=>$assessment_loop->get_item_from_extra_data('program_learning_outcome')?:-1
            ...
            if(count($courses)) {
                ...
                foreach (Orm_Course::get_all(['in_id' => $courses]) as $course) {
                    $selected = (!is_null($assessment_loop->get_item_from_extra_data('course')) && $course->get_id() == $assessment_loop->get_item_from_extra_data('course')) ? 'selected="selected"' : '';
                    ?>
                    <option
...
<script>
    ...
    function select_leaning_domain() {
        var data = $('#assessment-loop-form').serializeArray();
