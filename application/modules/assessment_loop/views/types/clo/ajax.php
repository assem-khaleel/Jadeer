...
if(is_null($learning_domain)) {
    $learning_domain = $assessment_loop->get_item_from_extra_data('learning_domain');
}
...
if(is_null($program_learning_outcome)) {
    $program_learning_outcome = $assessment_loop->get_item_from_extra_data('program_learning_outcome');
}
...
if(is_null($course)) {
    $course = $assessment_loop->get_item_from_extra_data('course');
}
        ...
        <tr>
            <td class="col-lg-3">
                <label class="radio" style="margin: 0 20px;">
                    <input type="radio" name="item_id" <?php echo $assessment_loop->get_item_id() === $clo->get_id() ? 'checked="checked"' : '' ?>value="<?php echo intval($clo->get_id()) ?>" class="px" />
