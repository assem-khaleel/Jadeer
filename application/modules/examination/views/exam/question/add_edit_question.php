...
/* @var $exam Orm_Tst_Exam_Questions*/
?>
...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        ...
        <?php echo form_open('/examination/save_question', 'id="question_form"') ?>
        <div class="modal-body">

            <div class="form-group">
                <label class="control-label"><?php echo lang('Question') ?></label>
                <input id="question_label" type="text" onclick="find_exam_questions(this,'question_id','question_label', <?php echo Orm_Tst_Exam::get_instance($exam_id)->get_course_id() ?>,<?php echo (int)$question->get_id() ?>)" readonly
            ...
            <div class="form-group">
                ...
                <input name="mark" id="mark" type="text" class="form-control"
                       value="<?php echo htmlfilter($exam->get_mark()); ?>"/>
                <?php echo Validator::get_html_error_message('mark'); ?>
            </div>
        </div>
        <div class="modal-footer">
            <input type="hidden" name="id" value="<?php echo urlencode($exam->get_id()); ?>">
            <input type="hidden" name="full_mark" value="<?php echo urlencode($full_mark); ?>">
            <input type="hidden" name="exam_id" value="<?php echo urlencode($exam_id); ?>">
...
<script>
    ...
    function find_exam_questions(element, property_id, property_label, course_id,question_id) {

        var url = (config.index_page ? '/' + config.index_page : '') + '/examination/find';

        $('.find_exam_questions').remove();

        var width = ($(element).width() + 26) + 'px';
        var position = $(element).position();

        $(element).after('<div id="wrapper_' + property_id + '" class="find_exam_questions" style="position: relative; z-index: 10; width: '+ width +'; left: ' + position.left + '; margin-top: 5px;">' +
            ...
            '<iframe style="border: none;" src="' + url + '?property_id='+property_id+'&property_label='+property_label+'&course_id='+course_id+'&question_id='+question_id+'" width="100%" height="420" ></iframe>' +
            '</div>' +
            '</div>' +
            '</div>');
    }
