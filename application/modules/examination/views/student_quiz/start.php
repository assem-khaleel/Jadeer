...
<div class="m-t-4 col-md-9 col-lg-9">
    <div class="panel panel-primary">
        <div class="panel-heading">
            <!--TODO quiz Name and timer will added here -->
            <div class="col-md-9 col-lg-9">
                <span class="font-weight-bold">
                    <span id="question_number">1</span> :
                    <span id="question_name"><?php echo $quiz->get_questions()[0]->get_question_id(true)->get_text() ?></span>
                </span>
            </div>
        </div>
        <div class="panel-body">
            <div id="question">
            <?php echo form_open('/examination/student_quiz/save_question/'.$quiz->get_id().'/'.$quiz->get_questions()[0]->get_question_id(), 'method="post"'); ?>
            <?php echo $quiz->get_questions()[0]->get_question_id(true)->get_question_with_user_response($quiz->get_id()) ?>
            <?php echo form_close(); ?>
            </div>
    ...
    <div class="panel panel-default panel-dark">
        ...
        <div class="panel-body">
            <div class="row" id="question_link">
                <?php foreach($quiz->get_questions() as $key=> $question): ?>
                <div class="col-md-3 col-lg-3 p-b-2">
                    <a data-question="<?php echo $question->get_question_id() ?>" href="javascript:void(0);" onclick="get_question(<?php echo $question->get_question_id() ?>);" class="btn btn-primary btn-outline btn-outline-colorless"><?php echo ($key+1) ?></a>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
...
<script>
    ...
    var question_count = <?php echo count($quiz->get_questions()); ?>;
    var stop=false;
    ...
    var check_time = function(){
        ...
        stop=true;
        $.ajax({
            url: '/examination/student_quiz/check_quiz/<?php echo $quiz->get_id() ?>'
        }).done(function (data) {
            if (!data.status) {
                window.location = '<?php echo base_url('/examination/student_quiz/timeout/'.$quiz->get_id()) ?>';
            }
            else {
                stop=false;
            }
    ...
    $('#finish_btn').click(function(){
        save_question(function () {
            window.location.href ='/examination/student_quiz/finish/<?php echo $quiz->get_id() ?>';
        })
    });
    ...
    function get_question(question_id){
        ...
        save_question(function(){
            $.get('/examination/student_quiz/get_question/<?php echo $quiz->get_id() ?>/'+question_id)
                .success(function(d){
                    if(d.success) {
