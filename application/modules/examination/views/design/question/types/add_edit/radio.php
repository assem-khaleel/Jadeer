...
<div class="form-group">
        ...
        if ($choices) {
            foreach ($choices as $choice) {
                if (is_object($choice)) {
                    ...
                    $params['choice_english'] = $choice->get_text_en();
                    $params['choice_arabic'] = $choice->get_text_ar();
                    $params['correct'] = $choice->get_correct();

                    $choice = $params;
                }
                $choice['correct'] = isset($choice['correct'])? $choice['correct']:0;
                ?>
                <div class="item">
                    <div class="row">
                        <div class="form-group col-md-1 text-center">
                            <input name="choices[<?php echo $index ?>][correct]" type="radio"  value=1 <?php echo (isset($choice['correct']) and $choice['correct'])? 'checked=""': '' ?>/>
                        </div>
                        ...
                        <div class="form-group col-md-2">
                            ...
                            <?php if ($index) { ?>
                                <button type="button" class="btn pull-right" aria-label="Left Align"
                                    ...
                                    <span class="fa fa-trash-o btn-label-icon left"
            ...
            <div class="item">
                <div class="row">
                    <div class="form-group col-md-1 text-center">
                        <input name="choices[0][correct]" type="radio" value=1>
                    </div>
                    <div class="form-group col-md-4">
    ...
    <div class="more_link">
        <button onclick="add_more();" aria-label="Left Align" class="btn" type="button">
            <span aria-hidden="true" class="fa fa-plus"></span> <?php echo lang('Add More'); ?>
        </button>
    </div>
    <?php echo Validator::get_html_error_message('correct_answer') ?>
</div>
...
<script type="text/javascript">
    function add_more() {
        ...
        $('#more').append('<div class="item">' +
            '<div class="row">' +
            '<div class="form-group col-md-1 text-center"><input name="choices[' + count + '][correct]" type="radio" value=1></div>' +
            '<div class="form-group col-md-4">' +
    ...
    $('div.more_items').on('click', 'input[type=radio]', function(){
        var input_tag = this;
        $('div.more_items input[type=radio]').each(function(){
            if(this==input_tag){
                return;
            }
