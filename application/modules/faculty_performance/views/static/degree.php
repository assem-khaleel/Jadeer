...
<div class="form-group" >
    ...
    <select class="form-control" name="ids[<?php echo intval($static->get_input()->get_id()) ?>][english]" >
        ...
        <?php foreach (Orm_Degree::get_all() as $degree) { ?>
            <?php $selected = $degree->get_id() == $static->get_result()->get_input_value_en() ? 'selected="selected"' : '' ?>
            <option value="<?php echo intval($degree->get_id()); ?>" <?php echo $selected ?>><?php echo htmlfilter($degree->get_name()); ?></option>
        <?php } ?>
    </select>
