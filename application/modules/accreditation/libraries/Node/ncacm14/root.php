...
class Root extends \Orm_Node
{
    ...
    protected $name = 'Course Specifications and Reports V.2015';
...
//    {
//
//        $node_obj = self::get_one(array('class_type' => self::COLLEGE_COURSE, 'item_id' => $item_id, 'system_number' => $this->get_system_number()));
//
//        if (!$node_obj->get_id()) {
...
//    }
    ...
    public function add_course($name, $item_id = 0, $assessor_ids = array())
    {
        ...
        $node_obj = self::get_one(array('class_type' => self::COURSE_COURSE, 'item_id' => $item_id, 'system_number' => $this->get_system_number()));
...
//        }
    ...
    public function system_validator(&$view_params = array())
    {
        ...
        $node = self::get_active_course_node();
        $node18 = self::get_active_ssr2018_node();
        if ($node->get_id() || $node18->get_id()) {
    ...
    public function collect_course_progress(&$finished = 0, &$in_process = 0, $filters = array())
    {
        ...
        //echo '<pre>' . self::get_model()->db->last_query() . '</pre>';
...
//echo $this->get_ci()->db->last_query();
        $progress = 0;
