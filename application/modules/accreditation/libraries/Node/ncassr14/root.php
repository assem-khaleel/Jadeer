...
class Root extends \Orm_Node
{
    ...
    protected $name = 'Self Study Reports V.2015';
    public $college_id;
    public $program_id;
    ...
    public function add_college($name, $item_id = 0, $assessor_ids = array())
    {
        ...
        $node_obj = self::get_one(array('class_type' => self::COLLEGE_SSR, 'item_id' => $item_id, 'system_number' => $this->get_system_number()));
    ...
    public function system_validator(&$view_params = array())
    {
        ...
        $node = self::get_active_ssr_node();
        $node18 = self::get_active_ssr2018_node();
        if ($node->get_id() || $node18->get_id()) {
            \Validator::set_error('common_error', lang('You can not have more than one SSR - Accreditations per 2 year.'));
        }
