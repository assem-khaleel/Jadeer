...
class Ssr_F extends \Orm_Node
{
    ...
    protected $name = 'F. Program Context';
    ...
    public function set_significant($value)
    {
        $property = new \Orm_Property_Textarea('significant', $value);
    ...
    public function get_significant()
    {
        return $this->get_property('significant')->get_value();
    }

    public function set_info()
    {
        $property = new \Orm_Property_Fixedtext('info', '2. Enrolment Management and Cohort Analysis (complete tables on the following pages) <br/> <br/>'
            . '<strong>Cohort Analysis </strong>nbsprefers to tracking a specific group of students who begin a given year in a program and following them until they graduate (How many students actually start a program and stay in the program until completion). <br/> <br/>'
            . '<strong>A cohort </strong>nbsprefers to the total number of students enrolled in the program at the beginning of each academic year, immediately after the preparatory year. No new students may be added or transfer into a given cohort. Any students that withdraw from a cohort may not return or be added again to the cohort. <br/> <br/>'
            . '<strong>Cohort Analysis Table 1</strong>nbspprovides complete tracking information for the most recent cohort to complete the program, beginning with their first year and tracking them until graduation (students that withdraw are subtracted and no new students are added). <br/> <br/>'
            . '<strong>Cohort of the Academic Year </strong> tables refer to current student cohort tracking that is in progress and have not graduated. A separate cohort tracking table should be provided for each year of enrolment (i.e., if the program is a 2, 4, or 5 year diploma program then there should be a table for each year, beginning with Table 1). '
            . '<strong>Enrollment Management and Cohort Analysis (Table 6) </strong>provides a summary of the cohort data given in each of the cohort analysis tables. Based on an assessment of all the data combined from each of the cohorts currently enrolled, there is space given to provide a critical analysis of the strengths or areas of concern identified for the whole program.');
        $this->set_property($property);
    }
    ...
    {
        $property = new \Orm_Property_Fixedtext('note', '<strong>NOTE: A separate table must be used for each branch/location campus.</strong>');
        $this->set_property($property);
    }
