...
class  Field_Experience_Sp_Signature extends \Orm_Node {
    ...
    public function set_signatures($value) {
        ...
        $instructor = new \Orm_Property_Fixedtext('instructor_name', 'Name of Field Experience Coordinator: ');
        $instructor->set_width(240);

        $signatures = new \Orm_Property_Fixedtext('signature', 'Signature: ');
        $signatures->set_width(240);

        $date_report = new \Orm_Property_Fixedtext('report_date', ' Date Specification Completed:');
        $date_report->set_width(240);

        $coordinator = new \Orm_Property_Fixedtext('coordinator', 'Program oordinator:');
        ...
        $property->add_cell(4, 1, $coordinator);
        $property->add_cell(4, 2, $name);

        $property->add_cell(5, 1,$signatures);
        $property->add_cell(5, 2, $signature);

        $property->add_cell(6, 1,$received);
        $property->add_cell(6, 2, $date);
