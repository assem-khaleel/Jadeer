<?php
/** @var $action_plans Orm_Acc_Pre_Visit_Reviewer_Action_Plan[] */
?>
    <div class="well well-sm">
        <?php echo htmlfilter(Orm_Acc_Pre_Visit_Reviewer::get_type_element($type, $type_id)->get_name()) ?>
        <hr class="bg-info">
        <?php echo xssfilter(Orm_Acc_Pre_Visit_Reviewer_Recommendation::get_instance($recommendation_id)->get_recommendation()) ?>
    </div>

    <div class="panel">
        <div class="panel-heading">
            <span class="panel-title"><?php echo lang('Action Plan List'); ?></span>
            <?php if($is_admin) { ?>
                <a href="/accreditation/reviewer_pre_visit/action_add_edit/<?php echo htmlfilter($type); ?>/<?php echo intval($type_id); ?>/<?php echo intval($recommendation_id); ?>" data-toggle="ajaxModal" class="btn pull-right" >
                ...
                <?php if ($action_plans) { ?>
                    <?php foreach ($action_plans as $action_plan) { ?>
                        <tr>
                            ...
                            <td>
                                <?php echo character_limiter(strip_tags($action_plan->get_description()),100,' ...') . ' <a href="/accreditation/reviewer_pre_visit/action_view/'.$action_plan->get_id().'" data-toggle="ajaxModal" >' . lang('Read More') . '</a>'; ?>
                            </td>
                            <td>
                                <?php
                            ...
                            <?php if($is_admin) { ?>
                                <td>
                                    <a href="/accreditation/reviewer_pre_visit/action_add_edit/<?php echo htmlfilter($type); ?>/<?php echo intval($type_id); ?>/<?php echo intval($recommendation_id); ?>/<?php echo intval($action_plan->get_id()) ?>"
                                    ...
                                    <a href="/accreditation/reviewer_pre_visit/action_delete/<?php echo intval($action_plan->get_id()) ?>"
