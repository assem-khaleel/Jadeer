...
class User_Employer_Model extends User_Model
{
    ...
    public function get_all($filters = array(), $page = 0, $per_page = 10, $orders = array(), $fetch_as = Orm::FETCH_OBJECTS)
    {
        ...
        $this->db->select(['ue.*', Orm_User::get_table_name().'.*']);
        $this->db->distinct();
        $this->db->from(Orm_User_Employer::get_table_name().' AS ue');
        ...
        switch ($fetch_as) {
            case Orm::FETCH_OBJECT:
                return Orm_User_Employer::to_object($this->db->get()->row_array());
                break;
            case Orm::FETCH_OBJECTS:
                $objects = array();
                foreach ($this->db->get()->result_array() as $row) {
                    $objects[] = Orm_User_Employer::to_object($row);
                }
                return $objects;
                break;
    ...
    public function get_filters($filters, $alias = 'ue')
    {
    ...
    {
        return $this->db->replace(Orm_User_Employer::get_table_name(), $params);
    }
    ...
    public function get_by_user_id($user_id)
    {
        ...
        $this->db->select('*');
        $this->db->distinct();
        $this->db->from(Orm_User_Employer::get_table_name());
