...
$filters = ['from' => $user_id, 'not_to' => $user_id];

$tasks = Orm_Tasks::get_all($filters, $page, $per_page);

$pager = new Pager(array('url' => '/tasks/sent_tasks'));
$pager->set_page($page);
...
<?php if ($tasks) { ?>
    <?php foreach ($tasks as $task) { ?>
        <div class="widget-tasks-item">
            <label class="custom-control custom-checkbox col-md-8 p-x-0">
...
<script>
    $('#my_tasks').removeClass('active');
    $('#sent_tasks').addClass('active');
</script>