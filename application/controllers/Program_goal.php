 ...
 *
 */
class Program_goal extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->view_params['page_header'] = $this->load->view('/common/page_header', array(
            'title' => lang('Program Goals') . ' : ' . htmlfilter($this->program->get_name()),
            'icon' => 'fa fa-gears'
        ), true);
    ...
    public function index()
    {
        ...
        $this->breadcrumbs->push(htmlfilter($this->program->get_name()), '/program_goal?program_id=' . $this->program->get_id());

        $html = '<div class="col-md-9 col-lg-10">';
        $html .= $this->program->draw_goals();
        $html .= '</div></div>';
